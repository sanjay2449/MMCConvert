...
// Handler
 const authinvoiceHandlerMulti = (req, res) => {
  uploadedData2 = [];
  ...
  try {
    ...
    const authInvoiceSheetName = 'AUTH_Invoice';
    ...
    if (!wb2.SheetNames.includes(authInvoiceSheetName)) {
      return res.status(400).json({ message: `"${authInvoiceSheetName}" sheet not found in second file.` });
    }

    const dataInvoiceNo = xlsx.utils.sheet_to_json(wb1.Sheets[invoiceNoSheetName], { defval: '' });
    const dataAuthInvoice = xlsx.utils.sheet_to_json(wb2.Sheets[authInvoiceSheetName], { defval: '' });

    unlinkSync(file1Path);
    unlinkSync(file2Path);

    if (!dataInvoiceNo.length || !dataAuthInvoice.length) {
    ...
    const updatedauthInvoice = dataAuthInvoice.map(row => {
...
// Download
 const authinvoiceDownloadMulti = (req, res) => {
...
export {
  authinvoiceHandlerMulti,
  authinvoiceConvertMulti,
  authinvoiceDownloadMulti,
};