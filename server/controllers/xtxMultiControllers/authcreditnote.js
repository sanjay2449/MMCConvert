...
const authcreditnoteHandlerMulti = (req, res) => {
  uploadedData2 = [];
  ...
  try {
    ...
    const authCreditNoteSheetName = 'Authcreditnote';
    ...
    if (!wb2.SheetNames.includes(authCreditNoteSheetName)) {
      return res.status(400).json({ message: `"${authCreditNoteSheetName}" sheet not found in second file.` });
    }

    const dataInvoiceNo = xlsx.utils.sheet_to_json(wb1.Sheets[invoiceNoSheetName], { defval: '' });
    const dataAuthCreditNote = xlsx.utils.sheet_to_json(wb2.Sheets[authCreditNoteSheetName], { defval: '' });

    unlinkSync(file1Path);
    unlinkSync(file2Path);

    if (!dataInvoiceNo.length || !dataAuthCreditNote.length) {
    ...
    const updatedAuthCreditNote = dataAuthCreditNote.map(row => {
    ...
    uploadedData2 = updatedAuthCreditNote;
    res.json({ message: 'Files uploaded and processed successfully.' });

  } catch (error) {
    console.error('Error in authcreditnoteHandler:', error);
    res.status(500).json({ message: `Failed to process files: ${error.message}` });
  }
};

const authcreditnoteConvertMulti = (req, res) => {
...
const authcreditnoteDownloadMulti = (req, res) => {
...
export {
  authcreditnoteHandlerMulti,
  authcreditnoteConvertMulti,
  authcreditnoteDownloadMulti,
};