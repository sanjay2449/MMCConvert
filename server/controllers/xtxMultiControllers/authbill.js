...
const authbillHandlerMulti = (req, res) => {
  uploadedData2 = [];
  ...
  try {
    ...
    const authBillSheetName = 'AUTH_Bill';
    ...
    if (!wb2.SheetNames.includes(authBillSheetName)) {
      return res.status(400).json({ message: `"${authBillSheetName}" sheet not found in second file.` });
    }

    const dataBillNo = xlsx.utils.sheet_to_json(wb1.Sheets[billNoSheetName], { defval: '' });
    const dataAuthBill = xlsx.utils.sheet_to_json(wb2.Sheets[authBillSheetName], { defval: '' });

    unlinkSync(file1Path);
    unlinkSync(file2Path);

    if (dataBillNo.length === 0 || dataAuthBill.length === 0) {
...
const authbillDownloadMulti = (req, res) => {
...
export  {
  authbillHandlerMulti,
  authbillConvertMulti,
  authbillDownloadMulti,
};
