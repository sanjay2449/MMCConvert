...
function transformInvoiceFields(row) {
  ...
  // Format Reference from Invoice_ID and Reference
  const billIdRaw = row['Invoice_ID'] || '';
  const originalReference = row['Reference'] || '';
  const trimmedBillID = billIdRaw.split('-')[0];

  transformed['Reference'] = originalReference
    ? `Inv Payment_${trimmedBillID}-${originalReference}`
    : `Inv Payment_${trimmedBillID}`;
...
// Upload Handler
const invoicepaymentHandlerMulti = (req, res) => {
  uploadedData = [];
...
// Convert Handler
const invoicepaymentConvertMulti = (req, res) => {
...
// Download Handler
const invoicepaymentDownloadMulti = (req, res) => {
...
export {
  invoicepaymentHandlerMulti,
  invoicepaymentConvertMulti,
  invoicepaymentDownloadMulti
};
