...
const type = "trackedadjustmentnote";
...
// Upload Controller
export async function uploadTrackedAdjustmentNote(req, res) {
  try {
    ...
    console.log("✅ Adjustment Note and COA merged and saved:", excelFilePath);
...
// Process Controller
// Process Controller
export async function processTrackedAdjustmentNote(req, res) {
  try {
    ...
    res.send({
      message: "Adjustment Note processed with removed Inventory entries.",
...
// Download Controller
export async function downloadTrackedAdjustmentNote(req, res) {
  try {
    if (await pathExists(modifiedExcelPath)) {
      res.download(modifiedExcelPath, "Modified_TrackedAdjustMentNote.xlsx", err => {
