...
let uploadedInvoiceFilePath = '';

const uploadInvoice = (req, res) => {
    uploadedInvoiceFilePath = req.file.path;
    console.log("Uploaded Invoice File Path: ", uploadedInvoiceFilePath);
    res.json({ message: 'Invoice file uploaded successfully.' });
};

const convertInvoice = async (req, res) => {
    try {
        if (!existsSync(uploadedInvoiceFilePath)) {
            return res.status(400).json({ error: 'Uploaded invoice file not found.' });
        }
        ...
        const originalRows = await parseCSV(uploadedInvoiceFilePath);
        const cleanedRows = [];
        let totalAmt = 0;
        ...
        for (const row of originalRows) {
            ...
            // True amount based on available DR/CR
            let trueAmt = 0;
            if (debit !== 0) {
                trueAmt = -debit;
            } else if (credit !== 0) {
            ...
            Object.entries(invoiceMapping).forEach(([sourceKey, targetKey]) => {
            ...
            allowedInvoiceColumns.forEach(col => {
                if (!(col in xeroRow)) {
...
export { uploadInvoice, convertInvoice, downloadInvoice };
