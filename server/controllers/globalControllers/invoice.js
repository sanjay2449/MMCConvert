...
//     "Product/Service Amount", "Product/Service Tax Code", "Product/Service Tax Amount", "Foreign Amount", "Product/Service Class",
...
//         row["Line Amount"] = (debit - credit).toFixed(2);
...
//         console.log("✅Global Invoice file saved at:", excelFilePath);
...
function processData(data) {
    return data.map(row => {
        ...
        if (row["Currency Code"] === currencyCode) {
            ...
            row["Line Amount"] = (debit + credit).toFixed(2); // Assuming foreign amount comes in "Debit" and "Credit"
        }
